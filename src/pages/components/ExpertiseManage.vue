<template>
  <div class="expertise-manage">
    ...
    <el-table :data="list" border stripe v-loading="loading" row-key="id">
      ...
      <el-table-column prop="name" label="专业能力名称" min-width="200" show-overflow-tooltip />
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="专业能力" prop="name">
          <el-input v-model="form.name" placeholder="请输入专业能力名称" />
        </el-form-item>
...
const rules = {
  name: [{ required: true, message: '请输入专业能力名称', trigger: 'blur' }]
}
...
const loadData = async () => {
  try {
    loading.value = true
    const res = await request.get('/expertise/list/all')
    list.value = res.data || []
  } catch (error) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (form.id) {
      await request.put(`/expertise/${form.id}`, form)
    } else {
      await request.post('/expertise', form)
    }
...
const handleDelete = async (id) => {
  try {
    await ElMessageBox.confirm('确定要删除这个专业能力吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })

    await request.delete(`/expertise/${id}`)
...
const moveUp = async (index) => {
  ...
  try {
    await Promise.all([
      request.put(`/expertise/${current.id}`, current),
      request.put(`/expertise/${prev.id}`, prev)
    ])
...
const moveDown = async (index) => {
  ...
  try {
    await Promise.all([
      request.put(`/expertise/${current.id}`, current),
      request.put(`/expertise/${next.id}`, next)
    ])
    ElMessage.success('排序更新成功')
    loadData()
  } catch (error) {
